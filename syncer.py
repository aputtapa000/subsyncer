...
class SubtitleSyncer:
    def __init__(self, master, video_path=None, srt_path=None):
        ...
        self.subs = self.parse_subtitles()
        self.current_sub = 0
    ...
    def parse_subtitles(self, path=None):
        subs = []
        try:
            ...
            if ext == ".srt":
                ...
                subs = self.parse_ass(srt_path)
            elif ext == ".sub":
                subs = self.parse_sub(srt_path)
            else:
        ...
        with open(path, 'r', encoding='utf-8') as f:
            for line in f:
                if line.startswith("Dialogue:"):
                    parts = line.split(",", 9)
                    start = self.time_to_ms(parts[1].strip())
                    end = self.time_to_ms(parts[2].strip())
                    text = parts[9].replace("\\N", "\n").strip()
                    subs.append({'start': start, 'end': end, 'text': text})
        return subs

    def parse_sub(self, path):
        subs = []
        with open(path, 'r', encoding='utf-8') as f:
            for line in f:
                parts = line.split(',')
                if len(parts) >= 3:
                    start = self.time_to_ms(parts[0].strip())
                    end = self.time_to_ms(parts[1].strip())
                    text = parts[2].strip()
                    subs.append({'start': start, 'end': end, 'text': text})
        return subs
    ...
    def save_subtitles(self):
        ext = os.path.splitext(self.synced_path)[1].lower()
        if ext == ".srt":
            self.save_srt()
        elif ext == ".ass":
            self.save_ass()
            ...
            print("Unsupported subtitle format for saving.")
    ...
    def save_ass(self):
        with open(self.synced_path, 'w', encoding='utf-8') as f:
            f.write("[Script Info]\n; Generated by SubtitleSyncer\n")
            f.write("[V4+ Styles]\n")
            f.write("[Events]\n")
            for sub in self.subs:
                f.write(f"Dialogue: 0,{self.ms_to_time(sub['start'])},{self.ms_to_time(sub['end'])},Default,,0,0,0,,{sub['text'].replace('\n', '\\N')}\n")

    def save_sub(self):
        with open(self.synced_path, 'w', encoding='utf-8') as f:
            for sub in self.subs:
                f.write(f"{self.ms_to_time(sub['start'])},{self.ms_to_time(sub['end'])},{sub['text']}\n")
...
if __name__ == "__main__":
    ...
    root.protocol("WM_DELETE_WINDOW", lambda: (app.save_subtitles(), app.cleanup(), root.destroy()))
